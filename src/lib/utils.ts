...
export type { CreditStatusDetails } from '@/lib/types';
...
export const decodeData = (encodedData: string): string => {
  // Si el dato parece ser una cédula en formato normal, no intentar decodificar
  if (/^\d{3}-\d{6}-\d{4}[A-Z]$/.test(encodedData) || 
      /^\d{3}-\d{2}'\d{3}-\d{4}[A-Z]$/.test(encodedData) ||
      /^\d{13,16}[A-Z]$/.test(encodedData)) {
    return encodedData;
  }
