...
function ColocacionReportContent() {
  ...
  // Group data by Sucursal only (removing supervisor grouping)
  const groupedData = reportData.reduce((acc, item) => {
    const sucKey = item.sucursalName || 'Sin Sucursal';
    if (!acc[sucKey]) {
      acc[sucKey] = [];
    }
    acc[sucKey].push(item);
    return acc;
  }, {} as Record<string, ColocacionRecuperacionItem[]>);
      ...
      <div className="report-container mx-auto">
        ...
        <div className="space-y-6">
          {Object.keys(groupedData).length > 0 ? (
            Object.entries(groupedData).map(([sucursalName, userList]) => {
              const sucursalTotalColocacion = userList.reduce((sum, item) => sum + item.colocacion, 0);
              const sucursalTotalRecuperacion = userList.reduce((sum, item) => sum + item.recuperacion, 0);
              const sucursalTotalDiferencia = userList.reduce((sum, item) => sum + item.diferencia, 0);
              const sucursalTotalDesembolsos = userList.reduce((sum, item) => sum + item.desembolsos, 0);
