...
export async function generateColocacionVsRecuperacionReport(filters: ReportFilters): Promise<ColocacionRecuperacionItem[]> {
    ...
    const allUsersSql = `SELECT id, fullName, sucursal_name FROM users WHERE active = true AND role IN ('GESTOR', 'ADMINISTRADOR', 'GERENTE')`;
...
export type { PercentPaidItem, ProvisionCredit } from '@/lib/types';
