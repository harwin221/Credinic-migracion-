...
export function ClientForm({ initialData }: ClientFormProps) {
  ...
  const asalariadoForm = useForm<AsalariadoFormValues>({
    ...
    } : { companyName: '', jobAntiquity: '', companyAddress: '' },
  });
              ...
              <Card className="shadow-md mt-4">
                <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    <GeographySelect
                      departmentValue={form.watch('departmentId')}
                      municipalityValue={form.watch('municipalityId')}
                      onDepartmentChange={(departmentId) => {
                        form.setValue('departmentId', departmentId);
                        form.setValue('municipalityId', ''); // Reset municipality when department changes
                      }}
                      onMunicipalityChange={(municipalityId) => {
                        form.setValue('municipalityId', municipalityId);
                      }}
