...
async function migrateUsersAndClients(oldDbConnection, newDbConnection, geoMaps) {
    ...
    for (const user of users) {
        ...
        if (userRole) { // Es un Usuario del Sistema
            ...
            console.log(`  [${SIMULATION_MODE ? 'SIM': 'REAL'}] Importando USUARIO: ${fullName} (ID Antiguo: ${user.id} -> ID Nuevo: ${newId})`);
...
async function migrateCredits(oldDbConnection, newDbConnection, userClientMap) {
    console.log(`--- FASE 2: Importando CRÉDITOS ---`);
    ...
    for (const credit of credits) {
        ...
        console.log(`  [${SIMULATION_MODE ? 'SIM': 'REAL'}] Importando CRÉDITO (ID Antiguo: ${credit.id} -> ID Nuevo: ${newId}) - Gestor: ${gestorName} - Sucursal: Sucursal Principal`);
        if (!SIMULATION_MODE) await newDbConnection.execute(sql, values);
    }
    if (skippedCount > 0) console.log(`  Se omitieron ${skippedCount} créditos por no encontrar a su cliente/usuario correspondiente.`);
    console.log('  ✅ Créditos importados.');
...
async function migratePayments(oldDbConnection, newDbConnection, creditMap, userClientMap) {
    ...
    const adminUserId = userClientMap[1]; // Obtenemos el ID nuevo del admin (legacyId: 1)
    let skippedCount = 0;
    ...
    for (const payment of payments) {
        ...
        const managedByNewId = userClientMap[payment.created_used_id] || adminUserId;
        ...
        const values = [
            ...
            managedByNewId, // Ya no puede ser null
            PAYMENT_STATUS_MAP[payment.estado] || 'valido'
        ];

        console.log(`  [${SIMULATION_MODE ? 'SIM': 'REAL'}] Importando PAGO (ID Antiguo: ${payment.id} -> ID Nuevo: ${newId}) para Crédito (ID Nuevo: ${newCreditId})`);
        if (!SIMULATION_MODE) await newDbConnection.execute(sql, values);
    }
...
async function runMigration() {
    let oldDbConnection;
    let newDbConnection;
    console.log('🚀 Iniciando el script de importación v6.0 (Lógica de Roles Definitiva)...');
